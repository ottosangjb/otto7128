...
$script:DownloadZipUrl     = "https://healthcheck.clarityit.com/download/healthcheck_script.zip"
...
try {
    ...
    # Gather sources (some may be unavailable depending on OS/permissions)
    $dd  = @(Get-CimInstance Win32_DiskDrive -ErrorAction SilentlyContinue)
    $pd  = @()
    try { $pd = @(Get-PhysicalDisk -ErrorAction SilentlyContinue) } catch { $pd = @() }

    # Map: Win32_DiskDrive DeviceID -> MSFT_Disk (serial/friendly/firmware on newer OS)
    $msftDisksByNumber = @{}
    try {
        $msft = @(Get-CimInstance -Namespace root/Microsoft/Windows/Storage -ClassName MSFT_Disk -ErrorAction SilentlyContinue)
        foreach ($m in $msft) {
            # MSFT_Disk.Number corresponds to Windows disk number
            if ($null -ne $m.Number) { $msftDisksByNumber[[int]$m.Number] = $m }
        }
    } catch { }

    if (-not $dd -or $dd.Count -eq 0) {
        Add-Line "  No physical disks found (or access denied)."
    } else {
        ...
        foreach ($d in $dd) {
            ...
            # Derive "Make" if Manufacturer is empty (common)
            if ([string]::IsNullOrWhiteSpace($vendor)) {
                # Try to infer from Model first token
                if (-not [string]::IsNullOrWhiteSpace($model)) {
                    $vendor = ($model -split '\s+')[0]
                }
            }

            # Get Disk Number from DeviceID like \\.\PHYSICALDRIVE0
            $diskNum = $null
            if ($d.DeviceID -match 'PHYSICALDRIVE(\d+)$') {
                $diskNum = [int]$Matches[1]
            }

            # Try enrich from MSFT_Disk (better serial/firmware on Win10/11)
            if ($null -ne $diskNum -and $msftDisksByNumber.ContainsKey($diskNum)) {
                $m = $msftDisksByNumber[$diskNum]

                # Some systems expose better values here
                if ([string]::IsNullOrWhiteSpace($serial) -and $m.SerialNumber) { $serial = $m.SerialNumber }
                if ([string]::IsNullOrWhiteSpace($firm)   -and $m.FirmwareVersion) { $firm = $m.FirmwareVersion }
                if ([string]::IsNullOrWhiteSpace($model)  -and $m.Model) { $model = $m.Model }

                # FriendlyName sometimes helps when Model is generic
                if ($m.FriendlyName -and ($model -notlike "*$($m.FriendlyName)*")) {
                    # keep model as-is; friendly name can be added separately if useful
                }
            }

            # Try enrich from Get-PhysicalDisk (Storage Spaces)
            $mediaType = $null
            $health    = $null
            if ($pd -and $null -ne $diskNum) {
                # Not a perfect join; try match by size & model fragment
                $match = $pd | Where-Object {
                    ($_.Size -and $d.Size -and $_.Size -eq $d.Size) -and
                    ($_.FriendlyName -and $model -and ($model -like "*$($_.FriendlyName)*" -or $_.FriendlyName -like "*$model*"))
            ...
            if ([string]::IsNullOrWhiteSpace($firm))   { $firm   = "Unknown" }

            $extras = @()
            if ($null -ne $diskNum) { $extras += ("Disk#{0}" -f $diskNum) }
            if ($mediaType) { $extras += ("Media={0}" -f $mediaType) }
            if ($health)    { $extras += ("Health={0}" -f $health) }

            $extraText = ""
            if ($extras.Count -gt 0) { $extraText = " | " + ($extras -join " | ") }

            Add-Line ("  Make={0} | Model={1} | Capacity={2} GB | Serial={3} | Firmware={4}{5}" -f `
                $vendor, $model, $sizeGB, $serial, $firm, $extraText)
        }
    }
    ...
    try {
        $vols = Get-Volume -ErrorAction SilentlyContinue | Where-Object { $_.DriveLetter -and $_.FileSystemType }
        foreach ($v in $vols | Sort-Object DriveLetter) {
            $sizeGB = if ($v.Size) { [math]::Round($v.Size/1GB,2) } else { 0 }
            Add-Line ("  {0}: | Label={1} | FS={2} | Size={3} GB" -f $v.DriveLetter, $v.FileSystemLabel, $v.FileSystemType, $sizeGB)
        }
    } catch {
        Add-Line "  Could not enumerate volumes."
    }
